...
Pair this with `chart.update(newData)` to build instant drilldown: clicking reports what was clicked, your application decides what each click means, and `update()` swaps the data without recreating the WebGL context. The library stays a renderer; the navigation pattern lives in your code. See the new **Interactions → Drilldown** page in the docs for a worked example.
